    ...
    try {
      ...
      for (const i in this.config.hosts) {
        ...
        for (const j in config.devices) {
          ...
          if (result.device === 'ds18b20') {
            if (!ds18b20.validId(result.id)) {
              this.warn(
                'config.json: hosts[%d]: devices[%d]: %s: not a valid DS18B20 ID',
                i, j, result.id
              )
              continue
            }
            result.fileName = ds18b20.fileName(result.id)
          }
